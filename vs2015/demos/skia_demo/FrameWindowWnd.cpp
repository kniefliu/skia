...
static inline bool decode_file(const char* filename, SkBitmap* bitmap, double scale = 1.0,
	...
	const SkImageInfo info = SkImageInfo::Make(size.width(), size.height(), kN32_SkColorType, alphaType);
...
LRESULT CFrameWindowWnd::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (!::IsIconic(*this)) {
		...
		if (layered_) {
			...
			if (offscreen_hbmp_)
				DeleteObject(offscreen_hbmp_);
			if (offscreen_hdc_)
				DeleteDC(offscreen_hdc_);
			...
			offscreen_hbmp_ = ::CreateDIBSection(offscreen_hdc_, &bmi, DIB_RGB_COLORS, (LPVOID*)&lpBitmapBits, NULL, 0);
...
LRESULT CFrameWindowWnd::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if (layered_) {
		...
		if (paint_window_)
			paint_window_->doPaint(offscreen_hdc_);
		...
		BOOL ret = ::UpdateLayeredWindow((HWND)m_hWnd, hDesktopDc, &ptDest, &sz,
			offscreen_hdc_, &ptSrc, 0, &bf, ULW_ALPHA);
